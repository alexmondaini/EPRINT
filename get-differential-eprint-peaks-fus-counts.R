...
### this step takes time. Do not run it everytime.

filename.df = read.table("D:/drabh/Dropbox/ePRINT/V0070/fus-eprint-bamfiles.txt")

### input bam files mapped to genes

filename.vec = filename.df$V1[filename.df$V3=="input"]
...
colnames(genecount.df) = c(colnames(genecount.df)[1:6], filename.df$V2[filename.df$V3=="input"])

write.table(genecount.df, file="D:/drabh/Dropbox/ePRINT/V0070/fus.input.gene.counts.txt", sep="\t", row.names = F, col.names = T, quote = F)
saveRDS(gene.fc, file="D:/drabh/Dropbox/ePRINT/V0070/fus.input.HK.gene.counts.rds")

### eprint bam files mapped to genes

filename.vec = filename.df$V1[filename.df$V3=="eprint"]
gene.fc = featureCounts(as.character(filename.vec), annot.ext = gene.df, isPairedEnd = T, strandSpecific = 1, 
                        countMultiMappingReads = F, requireBothEndsMapped = T, countChimericFragments = F)


genecount.df = cbind(gene.fc$annotation, gene.fc$counts)
colnames(genecount.df) = c(colnames(genecount.df)[1:6], filename.df$V2[filename.df$V3=="eprint"])

write.table(genecount.df, file="D:/drabh/Dropbox/ePRINT/V0070/fus.eprint.gene.counts.txt", sep="\t", row.names = F, col.names = T, quote = F)
...
colnames(pkcount.df) = c(colnames(pkcount.df)[1:6], filename.df$V2[filename.df$V3=="eprint"])
...
genecount.df = read.table("D:/drabh/Dropbox/ePRINT/V0070/fus.input.gene.counts.txt", sep="\t", header=T)
